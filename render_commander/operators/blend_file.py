...
class RECOM_OT_ExtractExternalSceneData(Operator):
    ...
    def _async_extract_scene_info(self, context, blend_path):
        ...
        def _task():
            ...
            blend_file_path = Path(blend_path)

            # --- CACHE LOGIC START ---
            current_blend_mtime = 0
            current_script_mtime = 0
            script_path = (Path(__file__).parent / "../utils/extract_scene_info.py").resolve()

            # Retrieve file modification times to validate cache state
            try:
                if blend_file_path.is_file():
                    current_blend_mtime = blend_file_path.stat().st_mtime
                if script_path.is_file():
                    current_script_mtime = script_path.stat().st_mtime
            ...
            try:
                ...
                path_hash = hashlib.md5(str(blend_file_path.resolve()).encode("utf-8")).hexdigest()
                cache_path = cache_dir / f"{path_hash}.json"
            except Exception as e:
                log.warning(f"Could not setup cache directory/path: {e}")

            extracted_info = None

            # Attempt to read existing valid cache
            if cache_path and cache_path.is_file() and current_blend_mtime != 0:
                try:
                    with open(cache_path, "r", encoding="utf-8") as f:
                        cache_data = json.load(f)
                        # Ensure both the blend file and extraction script haven't been modified
                        if (
                            cache_data.get("blend_mtime") == current_blend_mtime
                            and cache_data.get("script_mtime") == current_script_mtime
                        ):
                            extracted_info = cache_data.get("data")
                            log.debug(f"Loaded scene info from cache: {cache_path.name}")
                        else:
                            log.debug("Cache is outdated. Re-extracting scene info via background Blender.")
                except Exception as e:
                    log.warning(f"Failed to read cache file {cache_path.name}: {e}")
            # --- CACHE LOGIC END ---

            # If no cache or cache is invalid, run the extraction subprocess
            if extracted_info is None:
                extracted_info = _get_scene_info_from_subprocess(blend_path)

                # --- SAVE CACHE START ---
                if cache_path and extracted_info and "error" not in extracted_info and current_blend_mtime != 0:
                    try:
                        cache_data = {
                            "blend_mtime": current_blend_mtime,
                            "script_mtime": current_script_mtime,
                            "data": extracted_info,
                        }
                        with open(cache_path, "w", encoding="utf-8") as f:
                            json.dump(cache_data, f, indent=4)
                        log.debug(f"Saved scene info to cache: {cache_path.name}")
                    except Exception as e:
                        log.warning(f"Failed to write cache file {cache_path.name}: {e}")
                # --- SAVE CACHE END ---
